...
void* multiplyMatricial(void* arg) {
    for(int i = 0; i < tam; i ++){
        for(int j = 0; j < tam; j ++){
            for(int k = 0; k < tam; k++){
...
void* multiplyPosicional(void* arg) {
...
int main() {
    ...
    pthread_t t1;
    int a1 = 1;
    pthread_create(&t1, NULL, multiplyMatricial, (void *)(&a1));; //cria thread
    pthread_join(t1,NULL);

    pthread_create(&t1, NULL, multiplyPosicional, (void *)(&a1));; //cria thread
    pthread_join(t1,NULL);
